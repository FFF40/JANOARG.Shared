Shader "JANOARG/Highlight/Default"
{
    ...
    SubShader
    {
        ...
        // Separate alpha blend, carried over from Hit.shader: the highlight and its glow
        // also render into the options panel preview camera's RenderTexture, which is
        // cleared to alpha 0. Straight SrcAlpha/OneMinusSrcAlpha under-accumulates the
        // destination alpha there and the RawImage composite multiplies it in a second
        // time, darkening the glow. Against an opaque destination dstA is already 1 and
        // srcA + 1*(1-srcA) = 1, so the in-game player is unchanged.
